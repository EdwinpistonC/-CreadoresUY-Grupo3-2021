    ...
    public class GetFeedQuery : IRequest<Response<List<ContentDto>>>
    {
        ...
        public class GetFeedQueryHandler : IRequestHandler<GetFeedQuery, Response<List<ContentDto>>>
        {
            ...
            public async Task<Response<List<ContentDto>>> Handle(GetFeedQuery query, CancellationToken cancellationToken)
            {
                ...
                //TODO   orderby
                var content = await _context.Contents.Include(c=>c.ContentPlans).ThenInclude(cp=>cp.Plan).ThenInclude(p=>p.Creator).ThenInclude(c=>c.UserCreators).Where(c => c.ContentPlans.Any(cp=>listPlans.Contains(cp.IdPlan))  ||  (c.Public && c.ContentPlans.Any(cp=> cp.Plan.Creator.UserCreators.Any(uc=> uc.IdUser==query.IdUser && uc.Unfollow==false))) ).OrderByDescending(c=>c.AddedDate).Skip(query.Page*query.ContentPerPage).Take(query.ContentPerPage).ToListAsync();
